...
(function ($) {
  ...
  if ($('.scroll-move').length > 0){
    $(document).ready(function () {
      $(window).on("wheel", function (event) {      
          $(".scroll-move .scrolling-text").each(function (i, item) {
            if ($(item).isInViewport() && event.originalEvent.deltaY > 0) {
              $(item).animate({ left: "+=70px" }, 50, 'linear', function () {
                $(this).addClass('animated');
              });
            } else if($(item).hasClass('animated') && event.originalEvent.deltaY <=0){
              $(item).animate({ left: "-=70px" }, 50, 'linear');
            }else{
              $(this).removeClass('animated').css({left:'0px'});
            }
          });      
      });
    });
  ...
  if ($('.team-wrapper').length > 0){
      $(document).ready(function () {
        $(".team-wrapper").hover(
            function () {
                $(this).find('.author-name').addClass('border-bottom');
            },
            function () {
                $(this).find('.author-name').removeClass('border-bottom');
            }
        );
    });
  }
